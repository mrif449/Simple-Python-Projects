...
if command1 == 2:
    ...
    demo = input("Enter your sample for the test: ")
print(demo)

demo = demo.split()
start = time.time()
writing = input("Start Writing: ").split()
end = time.time()
keypress = 0
speed = 0
word = 0
elapsed = end-start
runner = len(writing)
for x in range(runner):
    if demo[x] == writing[x]:
        word += 1
    else:
        runner += 1
accuracy = (word/len(writing))*100
speed = (word*60)/elapsed
for x in writing:
    for y in x:
        keypress += 1
keypress += len(writing)
keypress -= 1
print("Result:")
...
exits = input("Press enter to close....")